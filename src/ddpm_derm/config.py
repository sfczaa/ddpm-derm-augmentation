"""Path and constant resolution.

All paths are derived from the project layout or overridden by environment
variables, so nothing is hard-coded to a personal machine. On Colab you only
need to set ``DDPM_DERM_DATA_DIR`` (and optionally ``DDPM_DERM_OUTPUTS_DIR``).

Data-dir resolution order:
  1. ``$DDPM_DERM_DATA_DIR``
  2. ``<project_root>/data``            (e.g. data copied into the project)
  3. ``<project_root>/../data``         (current desktop layout: data is a sibling)

A candidate is only accepted if it contains ``manifests/class_to_idx.json``.
"""
