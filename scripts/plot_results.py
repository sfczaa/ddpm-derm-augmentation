"""Plot classifier baseline results for eyeballing (torch-free).

Reads results_{variant}_seed{seed}.json produced by train_classifier and writes
PNGs to the figures dir. Primary metric is df F1; these plots are meant to help
judge C0 vs C1 (and later C4) at a glance, not for publication.

Needs only matplotlib + numpy, so it runs locally after you download the
results JSONs from Drive, or on Colab straight after aggregate_results.

Usage:
    python scripts/plot_results.py
    python scripts/plot_results.py --results-dir path/to/results --figures-dir path/to/figures
"""
