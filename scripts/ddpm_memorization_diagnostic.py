"""Nearest-neighbour diagnostics for the published synthetic df pool.

Compares synthetic-to-train distances with validation-to-train and
leave-one-out train-to-train reference distributions. Measurements use RGB
pixels and C1 ResNet-18 embeddings, including horizontal flips because the
generator used RandomHorizontalFlip during training.

Distances are descriptive indicators of similarity, not a proof of either
memorisation or novel lesion generation. Only train and validation manifests
are read; there are no pass/fail thresholds.
"""
