"""Is the washed-out synthetic df a sampling artefact or a trained-in property?

The failure-localisation diagnostic found the published synthetic df to be
desaturated (0.053 vs 0.188 for real df), low-contrast (0.064 vs 0.145), and
centred on RGB 0.5 -- the middle of the normalised range. That is the signature
of a sample regressing toward the data mean, and the resolution ladder showed
it is not a high-frequency/texture problem, so raising the generator resolution
would not address it.

The published set was drawn with DDIM at 50 steps and eta 0.0. Too few steps,
or a fully deterministic trajectory, can produce exactly this. That costs
nothing to test: the same checkpoint, resampled under other settings.

This sweep separates three explanations without retraining anything:

* sampler settings  -- vary num_steps and eta on one checkpoint.
* training progress -- run the baseline setting across saved epochs; if
  saturation is still climbing at epoch 100 the run simply stopped early.
* neither           -- if every configuration lands in the same place, the
  limitation is in the trained model and the next step is a real change.

Every configuration is measured with the same functions the published set was
measured with, so the numbers are directly comparable, and the real val df are
carried through as the reference scale. Read-only with respect to the fixed
split: train and val only, never test.
"""
